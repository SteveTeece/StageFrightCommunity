...
- [ ] T-061 Create shell layout component in `src/StageFright.UI/Shared/ShellLayout.razor` with dark brand strip (purple StageFright wordmark), white navigation bar, and module menu structure per section 1.3 architecture
- [ ] T-062 [P] Create MainLayout component in `src/StageFright.UI/Shared/MainLayout.razor` with two-column card layout and placeholder dashboard
...
- [ ] T-070 Create Add Member form component in `src/StageFright.UI/Pages/Members/AddMemberForm.razor` with fields: Name (required), Street Address (required), Phone (optional), Email (optional), Join Date (required), Date of Birth (optional) per FR-002
...
- [ ] T-095 Implement theme toggle component in `src/StageFright.UI/Shared/ThemeToggle.razor` with light/dark mode switch and Settings persistence per FR-019
