...
**Required-Constitution-Version**: 2.4.0  
...
**Resolution**: 5-second timeout is an **advisory target, NOT a hard SLA**. Reports generate synchronously with UI displaying modal "Generating report..." dialog until completion. No hard timeout enforced; if report takes >5 seconds, dialog continues waiting AND a Cancel button MUST appear allowing the user to abort and return to the Reports menu. **See FR-047 and NFR-019 for the authoritative cancel-button specification** — the "dialog continues waiting indefinitely" language from earlier drafts of this session is superseded by FR-047 (added 2026-06-11). Aligns with NFR-003 (no mandated SLAs).
...
A group coordinator schedules upcoming rehearsals and records member attendance. When attendance is recorded, the system automatically creates an attendance fee record (**paid by default**). The coordinator can view rehearsal history, attendance rates, and outstanding attendance fee balances.
...
2. **Given** a rehearsal is scheduled, **When** the user records attendance by selecting members present, **Then** attendance records are created and attendance fees are automatically marked as **paid** by default; coordinator may override individual members to unpaid via the "Mark as unpaid" checkbox before saving
...
2. **Given** the confirmation dialog is displayed, **When** the user confirms, **Then** annual fee records are created for all eligible active members (excluding inactive members and those with any existing annual fee record for the current year — paid or unpaid)
...
1. **Given** the Settings module is open, **When** the user navigates to Event Types, **Then** default types (Performance, Eisteddfod, Fund raiser, Promotional, Annual General Meeting) are displayed
...
**FR-002**: System MUST provide a Members module enabling user to create, edit, list, and filter members by Active/Inactive status; members MUST include: name (required), street address (required), phone (optional), email (optional), join date (required), and optional date of birth fields. System MUST validate email and phone formats when provided using the following patterns: **Email**: Use .NET `System.ComponentModel.DataAnnotations.EmailAddressAttribute` regex equivalent — validates presence of `@` and a domain with at least one dot (e.g., `user@example.com` valid; `noatsign` invalid; `@nodomain` invalid). **Phone**: North American or international format accepted — must match `^\+?[\d\s\-\(\)]{7,20}$` (allows optional leading `+`, digits, spaces, hyphens, and parentheses; minimum 7 digits, maximum 20 characters). Validation error messages: "Invalid email address format", "Invalid phone number format". When a new member is created via the "Add Member" form, the default Status is Active with ActivateDate set to today. Coordinator may immediately inactivate a member if needed.
...
**FR-003**: System MUST support member lifecycle through two orthogonal mechanisms: (1) **Status field** with two values: Active (member participates in events; fees apply) and Inactive (member exists but does not participate; no fees accrue). Marking a member Inactive MUST NOT set soft-delete fields and MUST NOT cascade to `CommitteeMembership` records — committee assignments remain intact on inactivation; Status is a participation indicator only. (2) **Soft-Delete (Archival)** via IsDeleted/DeletedAt/DeletedBy fields: when a member is archived (soft-deleted), IsDeleted is set to true and the member is hidden from default views but remains in database for historical reporting. Archival DOES cascade to current-year CommitteeMembership records (soft-delete). Both Active and Inactive members can be archived. This separates participation status (Status field) from data visibility/deletion (IsDeleted flag) per Constitution §3.5. Queries filtering non-archived members use: `WHERE IsDeleted=false`; queries filtering active participants use: `WHERE Status='Active' AND IsDeleted=false`.

**FR-004**: System MUST automatically apply annual membership fees to all active members (Status='Active') at the configured renewal month, skipping: (1) inactive members (Status='Inactive'), and (2) active members with any existing annual fee record for the current year (paid or unpaid). This prevents double-billing: once a member has been billed for the year, no second fee is generated regardless of whether the first fee has been paid. Coordinators who need to manually re-bill in exceptional circumstances must do so via a direct Finance transaction. Display a batch processing confirmation dialog showing the number of members to be charged. **Button Enable Rule**: The "Apply Annual Fees" button MUST be enabled only when the current calendar month >= the configured membership renewal month (e.g., if renewal month = 3 (March), button is disabled in January and February, enabled from March onward). Once enabled, the button remains enabled for the rest of the calendar year so coordinators can re-run if needed (re-runs are idempotent due to the existing-fee skip rule).

**FR-005**: System MUST provide a Rehearsals module enabling the user to schedule rehearsals (date, time, optional notes) and record attendance using a **batch interface per rehearsal**. Attendance recording screen displays: Member Name | [Attended ☐] | [Paid ☐] checkboxes for all active members on the rehearsal date, plus fee amount. Coordinator completes checkboxes for all members, then clicks Save/OK to atomically create all records. Recording attendance for active members (Status='Active') MUST automatically create attendance fee records with payment status defaulting to **PAID**. Override checkbox available at creation time: "Mark fee as unpaid (override)" allows coordinator to create fee record as UNPAID. Recording attendance for inactive members (Status='Inactive') is allowed for historical tracking but MUST NOT create fee records. **GL pair behavior at attendance recording**: When `PaidAtCreation=true` (default), the system creates TWO GL pairs atomically: (1) accrual pair — Debit MemberReceivable / Credit Income category (records the financial obligation); (2) payment pair — Debit Cash / Credit MemberReceivable (records cash received). The net effect on MemberReceivable is zero, so the fee shows a $0 outstanding balance immediately. Additionally, when `PaidAtCreation=true`, the system MUST auto-create a `Payment` record (`PaymentMethod=Cash`, `PaymentType=Attendance`, `Amount=Settings.AttendanceFee`, member reference) linked to the payment GL Transaction pairs, so Finance module payment history and Member Account Summary report display complete payment context for automatically-settled fees. When `PaidAtCreation=false` (override), only the accrual pair is created and no Payment record is generated; MemberReceivable carries a positive balance until the coordinator explicitly records a payment via the Finance module. This is the mathematical basis for `Outstanding = Σ(GL debits, member) − Σ(GL credits, member)`. After Save/OK, attendance records are **immutable** (Constitution §3.4) — no clearing or modification allowed in UI. If coordinator needs to correct a recorded attendance, they must use manual GL reversals via Finance module (debit MemberReceivable + credit to applicable Income category). Attendance records remain immutable and permanent in database.
...
**FR-013**: System MUST require a pre-import backup checkpoint and explicit user confirmation before any import writes data; import MUST be atomic (validate full payload first, then commit all changes in one transaction). The pre-import backup checkpoint file MUST be saved to the application data directory (same folder as the live database) with filename pattern `pre-import-backup-{timestamp:yyyyMMddHHmmss}.sfbak` (e.g., `pre-import-backup-20260623143022.sfbak`). The confirmation dialog MUST display the checkpoint file path so the coordinator knows where to find it for manual recovery.
...
1. Query all unpaid fees for member ordered by: FeeDate ASC (oldest first), then Fee.CreatedAt ASC (primary tiebreaker — always deterministic), then Fee.Id ASC (secondary tiebreaker — reliable only if PKs are time-ordered sequential GUIDs such as `Guid.CreateVersion7()`; **Implementation note**: if standard `Guid.NewGuid()` random GUIDs are used, omit `Fee.Id ASC` and rely solely on `Fee.CreatedAt ASC` to avoid non-deterministic ordering)
...
**FR-022**: System MUST implement audit trail logging (who, what, when) for all data modifications and retain logs for 12 months; purge expired logs on application startup only. If startup purge fails, log structured error and continue startup. **Retention Precision**: "12 months" is defined as 365 days; records with `CreatedAt < DateTime.UtcNow.AddDays(-365)` are eligible for purge. **Purge Timing**: Purge runs after EF Core database migrations complete but before the dashboard component renders, ensuring no partial-state UI is shown during cleanup.
...
**NFR-010**: **Tab Controls**: Every page except the dashboard that exposes multiple functions MUST use Blazor tab controls with accessible semantics (`role="tablist"`, `role="tab"`, `role="tabpanel"`). Module/plugin-contributed tabs MUST support deep-linking via query parameters. **Reserved `?tab=` values** (must not be reused across pages):

| Page | `?tab=` value | Notes |
|------|--------------|-------|
| Finance (`/finance`) | `balances` | Member balance list |
...
| Settings (`/settings`) | `restore` | Restore (TabKey="restore") |
| Plugin-contributed | `plugin-{pluginId}-{tabKey}` | Namespace pattern to avoid collisions |
...
**NFR-019**: **Report Generation Performance**: Report generation MUST be synchronous and blocking; user selects report, waits for data generation and display within common report viewer. UI displays modal dialog "Generating report..." with spinner throughout entire generation duration (modal always displayed, not conditional on timeout). No concurrent report generation. No caching between report selection, print, or export actions; each action triggers fresh data generation. No hard timeout enforced; if report takes >5 seconds, modal continues waiting AND a Cancel button MUST appear allowing the user to abort and return to the Reports menu (see FR-047 for the authoritative cancel-button requirement). **5-second target is advisory only** (see NFR-003) — reports may take longer for large datasets. Common report viewer displays modal until report data is ready or the user cancels, then modal closes and report (or Reports menu) displays.
...
- **No soft-delete fields**: Fee entity has no IsDeleted/DeletedAt/DeletedBy fields per Constitution §3.4 (financial records exempt from soft-delete pattern). Fee records are permanently immutable and are never soft-deleted.
...
- **Query Pattern**: Outstanding fees for member X are derived from GL transactions: `sum(GL debits where member=X) - sum(GL credits where member=X)`. Fee records themselves are never queried for paid/unpaid status; GL is the source-of-truth for outstanding balances.
...
**No Exceptions**: No grouping of any types in a single file is permitted, regardless of size or relatedness. This rule is NON-NEGOTIABLE per Constitution §3.2.1. Every public class, interface, record, struct, and enum MUST have its own file named exactly after the type. Private nested types are the only permitted exception.
...
- Q: Should member reactivation automatically forgive all prior outstanding fees or give coordinator choice? → A: **Coordinator choice with defaults (superseded by §1.1 Q4, 2026-05-20)**: When a coordinator marks an Inactive member as Active, the system displays a Reactivation Forgiveness dialog. Prior-year fees are pre-checked for forgiveness (default scope); current-year fees are shown unchecked (coordinator may override). Upon confirmation, GL reversing Transaction pairs (debit MemberReceivable, credit BadDebtExpense GL#9900) are created for each selected fee. Fee records are NOT soft-deleted (no soft-delete fields on Fee per Constitution §3.4). See FR-024 for full specification.

- Q: When a member is reactivated (Inactive→Active), which unpaid fees should be cleared and what record should be maintained? → A: **Write-Off with Audit Trail (GL reversals only — no soft-delete on Fee)**: Upon reactivation, system MUST: (1) Write off (forgive) all prior-period unpaid fees by creating GL reversing entries (debit on MemberReceivable, credit on BadDebtExpense/WriteOff category GL#9900) to zero-out selected balances; (2) Member's payable balance resets to reflect only non-forgiven fees; (3) Current-year membership fee is payable — the annual fee application process will generate a fresh current-year fee when applicable; (4) Log audit trail entries documenting which fees were forgiven. Original Fee records remain permanently in the database (no soft-delete fields on Fee per §6 Pass #3 Q5 and Constitution §3.4). GL reversals provide the complete audit trail of debt forgiveness.
...
- **Q1: Attendance Fee Creation & Payment Status (HIGH)** → **A: Option A with payment status default**
  ...
  - **No Clearing Mechanism**: Attendance records are permanently immutable after batch save (per §1.1 Q3 and FR-005). The ClearAttendance() workflow described in earlier drafts of this session is superseded by §1.1 Q3 (2026-05-20). Coordinators needing to correct attendance errors must use manual GL reversals in the Finance module.
  - **Rationale**: Aligns with typical performing arts practice (attendance = payment received); simplifies workflow (coordinator doesn't need to record both); reduces manual data entry.
  ...
  - **Impact on Functional Requirements**:
    - FR-005: Specifies automatic fee creation AND default-paid status; attendance records immutable after save
  ...
  - **Task Updates Needed**: T-054 (AttendanceService) implements fee creation with payment status default; T-076 (test AttendanceService) includes test cases for fee creation and payment status defaults
...
- **Q7: Committee Annual Reset Trigger Mechanism (HIGH)** → **SUPERSEDED by §1.1 Q6, 2026-05-20. See FR-031 for the authoritative specification.** Committee reset is manual-only (coordinator button in Settings > General tab); no automatic startup reset.

- **Q9: GL Transaction Recording Timing (CRITICAL)** → **A: Option A - Atomic transaction, Fee-first**
...
- Q: Are attendance records mutable after batch save, or permanently immutable? → A: Permanently immutable (Option A). No UI mechanism exists to clear or modify attendance records after batch save. The ClearAttendance() workflow described in §6 Pass #3 Q1 is removed — superseded by §1.1 Q3 (2026-05-20) and FR-005. Coordinators needing to correct attendance errors must use manual GL reversals in the Finance module.
- Q: Does the Fee entity include soft-delete fields (IsDeleted, DeletedAt, DeletedBy)? → A: No (Option A). Fee entity has no soft-delete fields. Constitution §3.4 exempts financial records from the soft-delete pattern entirely — the fields are not present on Fee, Payment, or Transaction entities. Key Entities §4 description updated; stale reactivation Q text updated. Fee records are permanently immutable; GL reversals provide all audit trail for forgiveness and corrections.
- Q: User Story 3 acceptance scenario 2 says attendance fees are "marked as unpaid" — should this be updated to reflect the clarified PAID default? → A: Yes. Scenario 2 updated to "marked as paid by default; coordinator may override individual members to unpaid via the 'Mark as unpaid' checkbox before saving." Aligns with §1.1 Q1, §6 Pass #3 Q1, and Q4.
- Q: When a member is marked Inactive (Status='Inactive'), should their current-year CommitteeMembership records be cascade-removed or remain unchanged? → A: Remain unchanged (Option A). Inactivation has no effect on CommitteeMembership records — committee assignments are only affected by the annual reset (FR-031) or archival (IsDeleted=true). FR-003 updated to make the inactivation/archival cascade distinction explicit.
- Q: Should the annual fee application skip members who already have an existing annual fee for the current year regardless of payment status, or only skip those with unpaid fees? → A: Skip if any annual fee record exists for current year, paid or unpaid (Option A). This prevents double-billing: once billed for the year, no second fee is generated. FR-004 and US4 acceptance scenario 2 updated accordingly. Coordinators needing exceptional re-billing must use a direct Finance transaction.
- Q: When PaidAtCreation=true at attendance recording, should the system auto-create a Payment record alongside the GL pairs? → A: Yes (Option B). When `PaidAtCreation=true`, AttendanceService MUST auto-create a `Payment` record with `PaymentMethod=Cash` (default), `PaymentType=Attendance`, `Amount=Settings.AttendanceFee`, and the member reference, linked to the payment GL Transaction pairs. This ensures the Finance module payment history and Member Account Summary report can display complete payment context (who paid, when, how) for automatically-settled attendance fees, consistent with the audit trail requirement of FR-016 and FR-022. When `PaidAtCreation=false`, no Payment record is created at attendance time; a Payment record is created only when the coordinator explicitly records a payment via the Finance module.
