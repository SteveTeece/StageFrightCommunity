...
- **Q7: Committee Annual Reset Trigger Mechanism (HIGH)** → **A: Manual button trigger (superseded by §1.1 Q6, 2026-05-20)**
  - **Decision**: Committee reset is a **manual coordinator action**, not an automatic startup check. See §1.1 Q6 and FR-031 for the authoritative specification.
  - **Summary of authoritative decision**: Coordinator clicks "Reset Committee for New Year" button in Settings > General tab. System displays confirmation dialog, then CommitteeAnnualResetService runs synchronously. No automatic startup reset. AGM banner displayed on Settings page if AGM occurred and reset has not been performed. Settings adds `CommitteeRenewalMonth` (int 1-12, default 1) and `LastCommitteeResetYear` fields.
  - **Note**: The startup-based automatic reset described in earlier drafts of this session is superseded. FR-031 is the authoritative requirement.
