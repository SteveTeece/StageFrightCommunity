...
1. **Given** the user is on the Finance Overview screen, **When** they view the available tabs, **Then** a "Record Expense" tab is present alongside the existing Outstanding, Record Member Payment, and Record Income tabs.
...
- **FR-008**: The Finance Overview screen MUST present a "Record Expense" tab alongside its existing tabs (Outstanding, Record Member Payment, Record Income, Apply Annual Fees).
...
- **SC-002**: 100% of accounts shown in the Chart of Accounts (active and archived) display a balance value — none are left blank or show an error.
