...
**Decision**: On a successful restore (first-run or Settings), navigate to a terminal `RestartRequiredScreen` at `@page "/restart-required"` under a **new chrome-free `BlankLayout`** (`src/StageFright.UI/Layout/`) — the same `CultureProvider` / `ThemeProvider` wrappers as `ShellLayout` but with **no** sidebar, no nav, and no theme toggle, so a post-restore user has no navigation surface at all. The screen itself renders only a "Close and reopen the application" instruction — **no** "Continue" / "Go to dashboard" / retry control. bUnit tests assert both (no escape control on the screen; no `.shell-sidebar` emitted by `BlankLayout`). The app does not relaunch itself. `ShellLayout` stays the only layout for every other route, first-run screens included; `BlankLayout` exists solely for this terminal screen.
...
**Alternatives considered**: a dismissible toast/alert then continue — rejected: does not satisfy "MUST NOT proceed … on pre-restore in-memory state". Keeping `/restart-required` on `ShellLayout` and relying on the screen not rendering its own escape controls — rejected: the shared sidebar is still a live navigation surface into pre-restore in-memory state, which FR-007 and the "Restore then immediately continue" Edge Case forbid; a dedicated `BlankLayout` removes the surface entirely. Auto-relaunch — rejected by the spec ("not reliably possible across all supported desktop platforms").
...
**Rationale**: `protobuf-net` binary is already culture-invariant and endian-defined, so text/number formatting cannot drift — but `DateTime.Kind` fidelity across time zones is the real risk, and the spec asks for an explicit test rather than an assertion of safety. New `DateTime` members on the new DTOs use the same plain `[ProtoMember]` default as the existing 20 `*BackupDto` types (protobuf-net's default `DateTime` wire form is a BCL tick+kind representation, not host-locale text), so the whole file stays one consistent format and the round-trip test is the guard that this is actually lossless. Per the CLAUDE.md note about `bunit-culture-mutation-test-race`, the culture-mutating test is placed in `StageFright.Integration.Tests` (not `UI.Tests`) and restores the ambient culture in a `finally`.

**Alternatives considered**: switching the new `DateTime` members to `DataFormat.WellKnown` (UTC `google.protobuf.Timestamp`) — rejected: it would make the new DTOs inconsistent with the 20 existing ones for no proven benefit, since the default form already round-trips; the round-trip test would catch a real problem if one existed.
