...
4. **Given** a restored database, **When** the application starts, **Then** any startup checks that compare a stored schema/version marker to the running application accept the restored data or migrate it forward, rather than treating it as unconfigured.
...
- **FR-003**: Before any database change, the system MUST validate the chosen backup file and show the user a summary of its contents — per-record-type counts, the date it was created, and the originating application version — with explicit confirm and cancel actions.
...
- **FR-011**: A backup MUST contain every piece of data required to reconstruct the application's current state on a fresh install — all member, attendance, rehearsal, event, committee, AGM, account, financial (fees, payments, GL transactions, GL journal entries, bank reconciliations and their lines), and audit-trail data — including archived (soft-deleted) records.
...
- **FR-018**: The existing Settings backup/restore screen MUST use the same default-filename rule (FR-010), the same post-restore restart advice (FR-007), and the same post-backup verification (FR-021–FR-024) as the first-run flow, so the behaviour is consistent wherever backup or restore is invoked.
