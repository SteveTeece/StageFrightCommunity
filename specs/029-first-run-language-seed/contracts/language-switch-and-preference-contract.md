...
- Synchronous. Sets `CultureInfo.DefaultThreadCurrentCulture`, `CultureInfo.DefaultThreadCurrentUICulture`, `CultureInfo.CurrentCulture`, `CultureInfo.CurrentUICulture` to `culture`; updates the public `CurrentCulture` property to `culture`; calls `StateHasChanged()`.
