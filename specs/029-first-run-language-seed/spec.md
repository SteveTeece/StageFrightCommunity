...
- **Sample-data seeding fails**: the error is surfaced on the first-run screen, the dashboard is not opened, and the app is not presented as ready. Recovering (e.g. wiping the partial database) is a developer action; this path is Debug-only.
