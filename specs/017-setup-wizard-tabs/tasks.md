...
- [x] **T037** [US2] Update `SetupWizard.razor(.cs)` — insert the Opening Balances tab at position 6 (after Chart of Accounts); queue state keyed by account reference (real `AccountId` or a queued account's `ClientId`), synced to the Chart of Accounts tab's queue (adding there adds a row here; removing there removes the row and any entered balance — FR-020); Finish populates `SetupRequest.QueuedOpeningBalances` + `OpeningBalanceAsAtDate`; Finish is blocked unless ≥1 non-zero queued balance OR the Review tab's seed-data checkbox is checked, with the coordinator able to tell the Opening Balances tab needs attention (FR-021); a failed Finish leaves both the balance queue and the account queue untouched (Edge Cases).

**⟶ Wait for Wave 2 to finish, then:**

**Wave 3 — independent (different files):**

- [x] **T038** [P] [US2] New `tests/StageFright.UI.Tests/Pages/Setup/Tabs/OpeningBalancesTabTests.cs` — rows for existing + queued accounts, negative amount, plug, sync when a queued account is added/removed on the Chart of Accounts tab.
- [x] **T039** [P] [US2] Update `SetupWizardTests.cs` — Finish blocked with no balance + sample-data unselected; Finish succeeds with a balance entered; Finish succeeds with sample-data selected and no balance entered (FR-021, US2 Acceptance Scenarios 2–3).
