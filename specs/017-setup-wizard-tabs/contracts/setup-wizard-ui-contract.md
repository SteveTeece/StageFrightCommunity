...
1. **Organisation Settings** — organisation name, annual fee, attendance fee, membership renewal month, audit retention years, then a two-column row pairing the theme dropdown (FR-022) with the tax-applicable checkbox, then (when tax is applicable) tax rate and annual/attendance fee tax treatment full-width below that row. Originally three separate tabs ("General", "Membership & Fees", "Sales Tax"); merged into one over two rounds so the wizard reads as fewer, denser steps — the four source components (`GeneralAppearanceTab`, `MembershipFeesTab`, `ThemeSelectionTab`, `SalesTaxTab`) are otherwise unchanged; `SetupWizard.razor` wraps the last two in a shared `row` so their (now `col-md-6`) root elements sit side by side.
2. **Chart of Accounts** — queued-account form (shared `AddAccountForm`) on its own row, at half width, with the list boxes below rather than beside it; a second row below pairs a read-only bordered list of every account that already exists (including seeded system accounts, each badged "System") side by side with the bordered list of queued accounts, each at half width — both scroll vertically within their own box (`BorderedListBox`'s `max-height`/`overflow-y`) rather than stacking.
...
4. **Committee** — two-column layout in two rows: AGM month paired with the seat count target on the first row; the office-holder role entry (+) field paired with its bordered list of queued titles on the second row. Moved to sit directly before Review — no other tab's data depends on Committee's, so relocating it is presentation-only.
