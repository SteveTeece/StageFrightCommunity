...
2. **Chart of Accounts** — two-column layout: queued-account form (shared `AddAccountForm`) on the left, unchanged at half width; on the right, a nested two-column row pairing a read-only bordered list of every account that already exists (including seeded system accounts, each badged "System") side by side with the bordered list of queued accounts — both scroll vertically within their own box (`BorderedListBox`'s `max-height`/`overflow-y`) rather than stacking.
