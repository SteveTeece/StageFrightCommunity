...
6. **Given** a Release build of the application, **When** I reach the Review & Finish step, **Then** the "Load sample data" checkbox and its progress overlay do not exist at all — there is never a database seed in Release, and Review & Finish otherwise behaves identically (still "Step 4 of 4").
...
- **FR-120**: The wizard's sample-data seeding progress moves from an inline spinner/text at the bottom of the form to a full-screen overlay (fixed position, dimmed backdrop, centered card) shown only once seeding actually starts (not during the preceding settings-creation step). Message: "Setting up your sample data — this may take a few minutes. Please don't close the app," plus the existing live progress text. New CSS class added to `app.css` (distinct from `ReportViewer`'s unstyled `modal-backdrop-light`, so that component's appearance is untouched). **This entire feature is Debug-build only**: `IDebugDataSeeder` is registered in `MauiProgram.cs` only inside `#if DEBUG`; `SetupWizard` resolves it optionally via `IServiceProvider.GetService` (not a required `[Inject]`) and renders the "Load sample data" checkbox and overlay only when the service is available. There is never a database seed in Release — the checkbox and overlay simply do not exist, and the wizard remains a 4-step flow either way.
...
- **SC-105**: In Debug builds, the sample-data seeding modal is visibly a blocking, full-screen "please wait" experience distinct from the brief settings-creation step that precedes it. In Release builds, no seeding checkbox or modal exists at all, and no database seed ever runs.
