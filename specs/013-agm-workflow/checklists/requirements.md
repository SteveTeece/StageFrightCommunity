...
- No `[NEEDS CLARIFICATION]` markers were needed — every open question from the source issue (custom office-holder titles, general-committee seat count, interaction with the existing AGM-as-event reminder banner) was resolved into an explicit, informed default under **Assumptions** in `spec.md`.
