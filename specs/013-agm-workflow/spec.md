...
- What happens if two AGMs are recorded in the same calendar year (e.g., an original meeting failed to reach quorum and was rerun)? Both meeting records are kept, each with its own committee position records exactly as originally saved; the most recently saved AGM's election results become the term's current committee positions, while the earlier AGM's own detail view continues to show what it recorded, unmodified.
...
- Q: When a coordinator saves a new AGM under the new workflow, does that save automatically close out the previous committee term (making the existing manual "click to reset" banner/action obsolete), or should that separate manual reset step still exist alongside the new AGM-save behavior? → A: AGM save fully replaces the old manual reset step — the existing "click to reset" banner and its underlying reset action are removed by this feature, since saving an AGM already performs the atomic supersede (FR-009, FR-010) that the manual reset used to do.
- Q: When a second AGM is saved for the same committee term (e.g., a rerun after a failed-quorum meeting) and it "supersedes" the first AGM's election results, are the first AGM's own committee-position records soft-deleted, or kept intact and simply no longer treated as current? → A: The first AGM's committee-position records are left untouched; each AGM's own detail view always shows exactly what that meeting recorded. "Current committee for the term" is computed as the results from the most-recently-saved AGM for that term, without altering or deleting any prior AGM's stored records.
- Q: Can a single custom office-holder title (e.g., "Vice President") have more than one member holding it at the same time within one committee term, or is each office-holder title — built-in or custom — limited to exactly one concurrent holder per term? → A: Every office-holder title, built-in or custom, holds exactly one member at a time per term, uniform with President/Secretary/Treasurer. A coordinator cannot assign two members to the same custom title within one AGM.
- Q: When a coordinator reviews a past AGM and it shows the general-committee seat-count target (e.g., "4 of 5 seats filled"), should that "5" be the target configured at the time that AGM was recorded, or always whatever the target is currently set to in Settings? → A: Snapshot the seat-count target onto each AGM record when it's saved. Past AGM review always shows the target as it was at that meeting, unaffected by later Settings changes.
...
- **FR-007**: The AGM screen MUST let the coordinator assign one member each to any additional office-holder positions defined for the organisation (User Story 2) — the same one-member-per-title rule as President/Secretary/Treasurer (FR-006) applies uniformly to custom titles — and record a member with no specific title as a general committee member.
...
- **FR-010**: Saving an AGM's election results MUST supersede any prior AGM's election results for the committee term it starts, for the purpose of determining the term's current committee — members not reselected in this AGM are no longer treated as currently holding a committee position for that term, unless already closed out earlier by a special election (User Story 4). Superseding MUST NOT alter or delete a prior AGM's own stored committee position records: each AGM's own detail view (FR-016) continues to show exactly what that meeting recorded, and "current committee" for the term is computed as the most-recently-saved AGM's results, not by editing or removing earlier AGMs' records.
...
- **FR-014**: The system MUST let the coordinator set a target number of general committee member seats, and the AGM screen MUST show how many of that target have been filled as selections are made. The seat-count target in effect at save time MUST be snapshotted onto the AGM record, so reviewing a past AGM (FR-016) always shows the target as it was when that meeting was recorded, unaffected by later changes to the Settings value.
...
- **FR-018**: The existing annual committee-reset reminder banner and its manual "reset" action MUST be removed as part of this feature — saving an AGM (FR-009, FR-010) already performs an atomic supersede of the prior committee term's position records, making the separate manual reset step unnecessary and obsolete.
...
- **Annual General Meeting**: Represents one AGM sitting — meeting date, optional notes, the general-committee seat-count target snapshotted at save time (see FR-014), and the standard archive/audit fields. Owns its attendance and, indirectly (via the committee position records for its resulting committee term), its election results.
- **AGM Attendance Record**: Records whether one member attended one specific AGM. One record per (AGM, member) pair.
- **Committee Office-Holder Type**: A defined position title an organisation can elect members into at an AGM (e.g., "Vice President"), beyond the three built-in positions. Has a display order and can be archived. Like the built-in positions, exactly one member may hold a given title at a time within a committee term.
- **Committee Position Record** *(existing — reused, extended)*: The elected outcome for one member, in one named position (or blank for a general committee member), for one committee term (an AGM-to-AGM cycle, not necessarily a calendar year — see User Story 3). Carries a start date and an optional end date, so a term with a mid-term replacement (User Story 4) can hold more than one record for the same position, each dated to when that person actually served. When more than one AGM is recorded for the same committee term (e.g., a rerun), each AGM's own committee position records are preserved unmodified; which AGM's records count as the term's current committee is determined by recency of the owning AGM, not by deleting or editing earlier records.
