...
- [ ] **T071** Run `dotnet build` and the full `dotnet test` suite (all five projects, no `--no-build`) and confirm every test is green; manually verify via `dotnet run --project src/StageFright.App/` that Record AGM → Past AGMs → AGM Detail → Special Election flows work end-to-end in the running app
