...
- Q: Should the "Rehearsal Fee Paid" checkbox column keep its label, or show something else? → A: Its header is replaced by the per-rehearsal attendance fee amount (sourced from Settings), formatted as currency with no decimal places (e.g. "$5"), so the attendance-taker can see what each member owes without a separate lookup. The checkbox itself keeps real payment-status semantics (see FR-007).
...
- **FR-006**: Each member row MUST include a fee checkbox column whose column heading is the configured per-rehearsal attendance fee amount (Settings' attendance fee), formatted as currency with zero decimal places (e.g. "$5"), rather than descriptive text. If no attendance fee has been recorded yet for that member for that rehearsal, the checkbox MUST print unchecked. If an attendance fee has been recorded, the checkbox MUST show checked only when that fee has no outstanding balance, and unchecked otherwise (including when the member was marked attended but the fee was marked unpaid).
...
- **Settings**: Existing entity. Supplies the configured attendance fee amount shown in the fee column's heading.
...
- The attendance fee amount shown in the fee column's heading is the value currently configured in Settings at generation time (not a historical snapshot from the rehearsal's date), since it is a per-rehearsal amount that is not expected to change often.
