...
| **Residual-literal scan** (`Should_HaveNoUserFacingLiteral_When_ConvertedFileScanned`) | Over the file list a given phase has converted (US1 list, then US2 list), assert no user-facing string literal remains — element text nodes, `aria-label` / `alt` / `title` attribute values (FR-001), and `Text`/`Title`/`Label`/`Placeholder`/`Header`/`_errorMessage`/`_successMessage` assignments. Allow-list: log-message strings, routes (`"/members"`), CSS classes, format tokens (`"N2"`, `"dd/MM/yyyy"`), enum/filter value **tokens** used as `<option value>` / persisted keys (`"Active"`), `data-*` attributes, keys themselves, and genuinely decorative `alt=""` / `aria-hidden` subtrees. Scoped per phase so US1 can be green before US2 (SC-001). |
