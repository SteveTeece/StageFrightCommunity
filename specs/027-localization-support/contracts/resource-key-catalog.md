...
9. **Currency / number formatting** (FR-015, resolved 2026-08-27): monetary amounts are **not** resource strings — they are produced by the shared `MoneyFormatter` (`localization-contracts.md` §8), which keeps the `"$"` / `"AUD"` symbol fixed while separators, grouping and placement follow `CultureInfo.CurrentCulture`. Never format a display amount with `.ToString("C")` / `"{0:C}"` / `FormatString="{0:C}"` — that substitutes the active culture's currency symbol. **Superseded by spec 028:** the symbol/code/precision come from the organisation's configured currency (`Settings.CurrencyCode`, default `AUD`), not a hard-coded `"$"`.
