...
Menu/tile providers (`StageFright.Core`) take `IStringLocalizer<NavigationResource>` via constructor and populate `MenuItem.Title` in `GetMenuItems()`. Report providers/renderers (`StageFright.Reports`) take `IStringLocalizer<ReportsResource>` via constructor, and use `LocalizeEnum` for any enum-valued cell (e.g. `m.Status` in the Member List report). Report filter definitions keep the invariant token as the option value and set the option label from `EnumsResource` (`Enum_MemberStatus_Active`) or, for a synthetic option, `ReportsResource` (`Reports_Filter_OptionAll`).
