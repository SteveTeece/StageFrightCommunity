...
- Culture-sensitive formatting of dates and numbers follows the selected region, and parsing of user input follows the same region; stored and serialized values (database, backups, GL) remain culture-invariant. Monetary amounts are always Australian dollars: the "$" / "AUD" symbol/code is fixed regardless of the UI language, and only the number's separators, grouping and symbol placement follow the selected region (FR-015, resolved in Clarifications, Session 2026-08-27).
