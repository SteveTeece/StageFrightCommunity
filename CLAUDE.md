...
**A new or changed neutral key ships fully translated in the same change.** Every key added to — or reworded in — a neutral `.resx` MUST also be added, with a real translation (never the English value left in place), to *every* shipped culture set beside it: `de-DE`, `en-US`, `es-ES`, `fr-FR`, `it-IT`, `ja-JP`, `pl-PL`. `en-AU` needs no separate file — the neutral `.resx` **is** the `en-AU` baseline — and `qps-ploc` is regenerated by `scripts/generate-pseudo-locale.py`, never hand-edited. No shipped culture `.resx` may lack a key its neutral file defines — build-enforced by `Us2LocalizationGuardTests.Should_TranslateEveryNeutralKey_When_ShippedCultureResxScanned` (`qps-ploc` excluded); the key-by-key fallback to `en-AU` is a regression safety net, not licence to ship a language with gaps.
...
**Money is never `.ToString("C")` / `{0:C}` / `FormatString="{0:C}"` / `ToString("F2")` at a display site** — the `"C"` specifier substitutes the active culture's currency symbol (e.g. `€` under `fr-FR`), misrepresenting the amount. Every displayed, printed or exported amount goes through `MoneyFormatter` (`Format` / `FormatWithCode`). Since spec 028 the symbol, ISO code and minor-unit precision come from the organisation's configured currency (`Settings.CurrencyCode`, ISO 4217, chosen once at first-run setup, default `AUD`/`$`/2 digits) — set process-wide via `MoneyFormatter.Configure(...)` in `MauiProgram` right after the display culture, exactly like the startup culture. Only separators, grouping and symbol placement follow `CultureInfo.CurrentCulture`; a negative amount always renders with a leading/trailing minus sign (matching that culture's symbol placement), never accounting-style parentheses — `MoneyFormatter` pins `NumberFormatInfo.CurrencyNegativePattern` so `Format(-42.10m)` is `"-$42.10"` on every host, not the invariant culture's default `"($42.10)"`. An `AUD` dataset is byte-identical to the pre-028 output. `CurrencyCatalog` (`StageFright.Core/Localization/`) is the curated ISO 4217 list (spans 0-, 2- and 3-minor-digit currencies); `TaxCalculator.SplitInclusive` / `SplitExclusive` round tax to the configured currency's minor unit. Since spec 028 issue #354, `Settings.TaxEntryMode` (`Inclusive` — default, every pre-#354 dataset — or `Exclusive`) decides whether a newly entered taxable figure is the tax-inclusive gross or the net with tax added on top; the taxable posting services (`FeeService`, `IncomeEntryService`, `ExpensePaymentService`, `AttendanceService`) dispatch through `TaxCalculator.Split(entered, mode, rate, digits)` → `(gross, net, tax)`, and in `Exclusive` mode the receivable/bank line (and `Fee.Amount` / `Payment.Amount`) carry the gross. `Inclusive` mode is byte-identical to pre-#354; no GL-structure, `2310`/`2320` or `TaxCode` change.

Display language resolves at startup via `LanguageProvider` (explicit `Settings.LanguageCode` → recorded no-database language preference (`ILanguagePreferenceStore`, MAUI `Preferences`, spec 029) → OS display language when a matching set ships → `en-AU`) and is set as the process culture before the `BlazorWebView` renders. The supported-language list is discovered at runtime from shipped `.resx` cultures (`qps-*` pseudo-locales excluded), so adding a language is a drop-in `<Marker>.<culture>.resx` set with **no code change**. Since spec 029 a language change applies **live, in the running session, with no restart and no "restart required" notice anywhere**: `CultureProvider.Switch(CultureInfo)` (`src/StageFright.UI/Layout/CultureProvider.razor.cs`) reassigns the two process-wide culture globals (`CultureInfo.DefaultThreadCurrentCulture` / `DefaultThreadCurrentUICulture`) — and **only** those, never the `AsyncLocal`-backed `CultureInfo.CurrentCulture` / `CurrentUICulture`, which as a per-execution-context override would shadow the globals on the renderer's context and freeze the UI language until a process restart (spec 029's T036 defect); `MauiProgram.RunStartupSequence` follows the same "globals only" rule — then re-renders the whole cascaded tree. Both entry points — the first-run `/language-select` screen and Settings → General — call `Switch` and also persist the choice (`ILanguagePreferenceStore.Set` plus, once the DB exists, `Settings.LanguageCode`). Full contributor/translator guide: [`docs/localization/adding-a-language.md`](docs/localization/adding-a-language.md).
...
`.github/workflows/debug-pre-release.yml` (manual-only, `workflow_dispatch`; was `debug-msix-release.yml` before it grew a macOS job) builds **Debug**-configuration packages of the `dev` branch for two platforms and publishes them together as the single rolling `dev-debug-build` pre-release (constant asset filenames, never accumulates history): `StageFrightCommunity-dev-DEBUG.msix` (Windows, self-signed MSIX) and `StageFrightCommunity-dev-DEBUG-mac.zip` (macOS, an **unsigned, universal** Mac Catalyst `.app` — `-p:CreatePackage=false -p:PublishMacUniversal=true`, zipped with `ditto`, no signing secrets). The universal (x64 + arm64) build hinges on `RuntimeIdentifiers` (**plural**) being `maccatalyst-x64;maccatalyst-arm64`, and that value **must be set inside `StageFright.App.csproj`**, never on the CLI: `-p:RuntimeIdentifier` (singular) rejects a `;`-list with `NETSDK1083: ... 'maccatalyst-x64;maccatalyst-arm64' is not recognized`, and `-p:RuntimeIdentifiers` (plural) on the CLI becomes a global property that leaks the `;`-list verbatim into every `ProjectReference` (the plain non-maccatalyst class libraries), which then fail the same `NETSDK1083`. So the workflow passes `-p:PublishMacUniversal=true`, which flips a conditional `<PropertyGroup>` in `StageFright.App.csproj` (gated on `PublishMacUniversal == 'true'` **and** a maccatalyst TFM — inert otherwise, so local `dotnet build`/`dotnet run` maccatalyst builds stay single-arch) that sets the plural `<RuntimeIdentifiers>`. As a project property MAUI fans out per-RID inner builds that hand a single concrete RID to the referenced libraries. Release builds get this universal RID from MAUI automatically; a Debug build has to opt in. Mirrors dotnet/maui's own `Controls.TestCases.HostApp.csproj` CI pattern. (Issue #366 was the earlier, wrong fix — a `%3B`-escaped CLI `-p:RuntimeIdentifier`; the `%3B` escaping mattered only because `-p:` treats a literal `;` as a property-list delimiter, but the CLI approach itself was the dead end.) It is three jobs: `build-windows-msix` and `build-mac-app` run in parallel and each upload their package as a CI artifact; `publish-release` `needs:` both, downloads the artifacts, and does the single `gh release delete dev-debug-build --cleanup-tag` then `gh release create` with both assets — a full delete + recreate, *not* `gh release upload --clobber`, because GitHub "immutable releases" (GA Oct 2025) permanently block replacing/deleting *or adding* an asset once a release is published (`--clobber` fails with `HTTP 422: Cannot delete asset from an immutable release`) while deleting a whole release stays allowed; the delete + recreate works whether or not repo-level release immutability is enabled. Because `publish-release` needs both builds, a break on **either** platform's tooling means no release that run (the previous one stays intact) rather than a half-updated release. If a `dev-debug-build` release was already published as immutable, it must be deleted once by hand — disabling the repo setting does not retroactively unlock it. Only the Windows job needs repo secrets: `MSIX_CERT_BASE64` (a self-signed code-signing cert, base64-encoded PFX bytes) and `MSIX_CERT_PASSWORD`. The cert's Subject **must exactly match** `Identity/@Publisher` in `Platforms\Windows\Package.appxmanifest` (currently `CN=StageFright Community`) or MSIX packaging fails — if that Publisher value is ever changed, a matching new cert must be generated and the secret rotated, and vice versa. The Mac `.app` is only ad-hoc-signed, so testers must clear the Gatekeeper quarantine flag (`xattr -dr com.apple.quarantine "StageFright Community.app"`, or right-click → Open); note `Platforms/MacCatalyst/Info.plist` still carries the MAUI-template default `UIRequiredDeviceCapabilities` = `arm64`, which could keep the Intel slice of the universal build from launching on Intel Macs — revisit that key if Intel support actually matters. `StageFright.App.csproj`'s local/CI `Release`+`Debug` builds are otherwise untouched (Windows still unpackaged, `WindowsPackageType=None`) — the workflow passes `WindowsPackageType=Package`/signing purely as `dotnet publish -p:` command-line overrides. The project file carries two conditional `PropertyGroup`s that are inert for every normal build and only fire when the workflow's own `-p:` flags set them: `RuntimeIdentifierOverride`→`RuntimeIdentifier` (the documented workaround for [WindowsAppSDK#3337](https://github.com/microsoft/WindowsAppSDK/issues/3337), tripped by the Windows job's `-p:RuntimeIdentifierOverride=win-x64`) and `PublishMacUniversal`→plural `RuntimeIdentifiers` (tripped by the Mac job's `-p:PublishMacUniversal=true`).
